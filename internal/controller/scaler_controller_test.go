...
var _ = Describe("Scaler Controller", func() {
	Context("When reconciling a resource", func() {
		const resourceName = "scaler-sample"
		...
		BeforeEach(func() {
			...
			fmt.Println("getting scaler....")
			if err != nil && errors.IsNotFound(err) {
				fmt.Println("Got error....1")
				resource := &scalerv1alpha1.Scaler{
				...
				fmt.Println("Got error....2")
				Expect(k8sClient.Create(ctx, resource)).To(Succeed())
				fmt.Println("Got error....3")
			}
		})
